...
struct FCompiledCameraModel
{
	 ...
	 */
	FVector4f DistortedCameraMatrix;
	FVector4f UndistortedCameraMatrix;

	/** Output multiply and add of the channel to the render target. */
	FVector2f OutputMultiplyAndAdd;
};


/** Undistorts top left originated viewport UV into the view space (x', y', z'=1.f) */
static FVector2f LensUndistortViewportUVIntoViewSpace(
	const FFooCameraModel& CameraModel,
	float TanHalfDistortedHorizontalFOV, float DistortedAspectRatio,
	FVector2f DistortedViewportUV)
{
	FVector2f AspectRatioAwareF = CameraModel.F * FVector2f(1, -DistortedAspectRatio);
...
IMPLEMENT_SHADER_TYPE(, FLensDistortionUVGenerationVS, TEXT("/CustomShaders/MyShader.usf"), TEXT("MainVS"), SF_Vertex)
IMPLEMENT_SHADER_TYPE(, FLensDistortionUVGenerationPS, TEXT("/CustomShaders/MyShader.usf"), TEXT("MainPS"), SF_Pixel)
...
FVector2f FFooCameraModel::UndistortNormalizedViewPosition(FVector2f EngineV) const
{
	// Engine view space -> standard view space.
	FVector2f V = FVector2f(1, -1) * EngineV;

	FVector2f V2 = V * V;
	float R2 = V2.X + V2.Y;

	// Radial distortion (extra parenthesis to match MF_Undistortion.uasset).
	FVector2f UndistortedV = V * (1.0 + (R2 * K1 + (R2 * R2) * K2 + (R2 * R2 * R2) * K3));
	...
	// Returns engine V.
	return UndistortedV * FVector2f(1, -1);
}
	...
	float DistortedHorizontalFOV, float DistortedAspectRatio) const
{
	...
	//     
	//        6        5        4
	FVector2f UndistortCornerPos0 = LensUndistortViewportUVIntoViewSpace(
		*this, TanHalfDistortedHorizontalFOV, DistortedAspectRatio, FVector2f(0.0f, 0.0f));
	FVector2f UndistortCornerPos1 = LensUndistortViewportUVIntoViewSpace(
		*this, TanHalfDistortedHorizontalFOV, DistortedAspectRatio, FVector2f(0.5f, 0.0f));
	FVector2f UndistortCornerPos2 = LensUndistortViewportUVIntoViewSpace(
		*this, TanHalfDistortedHorizontalFOV, DistortedAspectRatio, FVector2f(1.0f, 0.0f));
	FVector2f UndistortCornerPos3 = LensUndistortViewportUVIntoViewSpace(
		*this, TanHalfDistortedHorizontalFOV, DistortedAspectRatio, FVector2f(1.0f, 0.5f));
	FVector2f UndistortCornerPos4 = LensUndistortViewportUVIntoViewSpace(
		*this, TanHalfDistortedHorizontalFOV, DistortedAspectRatio, FVector2f(1.0f, 1.0f));
	FVector2f UndistortCornerPos5 = LensUndistortViewportUVIntoViewSpace(
		*this, TanHalfDistortedHorizontalFOV, DistortedAspectRatio, FVector2f(0.5f, 1.0f));
	FVector2f UndistortCornerPos6 = LensUndistortViewportUVIntoViewSpace(
		*this, TanHalfDistortedHorizontalFOV, DistortedAspectRatio, FVector2f(0.0f, 1.0f));
	FVector2f UndistortCornerPos7 = LensUndistortViewportUVIntoViewSpace(
		*this, TanHalfDistortedHorizontalFOV, DistortedAspectRatio, FVector2f(0.0f, 0.5f));

	// Find min and max of the inner square of undistorted Viewport in the view space at z'=1.
	FVector2f MinInnerViewportRect;
	FVector2f MaxInnerViewportRect;
	...
	FVector2f ViewportScaleUpFactorPerViewAxis = 0.5 * FVector2f(

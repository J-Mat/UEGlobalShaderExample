...
TArray<FVector3f> AMeshActor::GetVerts(AActor* Actor)
{
	...
	for (uint32 VertIdx = 0; VertIdx < VertexBuffer->GetNumVertices(); VertIdx++)
	{
		...
		FVector VertexWS = Actor->GetTransform().TransformPosition(FVector(VertexLS.X, VertexLS.Y, VertexLS.Z));

		// Add it to the array we'll return
		MeshVerts.Add(FVector3f(VertexWS.X, VertexWS.Y, VertexWS.Z));	// NOTE: .Add can be pretty slow!
	}
